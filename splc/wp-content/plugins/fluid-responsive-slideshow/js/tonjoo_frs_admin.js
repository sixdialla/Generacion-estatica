jQuery(document).ready(function($){
	...
	$('.frs-modal-container').on('click','[frs-save-slider]',function(){
	 	...
	 	else
	 	{
			...
			var data =  $('#frs-modal-form').serialize() + '&action=frs_save&content=' + content +"&post_id="+post_id+"&slide_type="+current_frs_slide_type+"&nonce="+nonce;
			...
			$.post(ajaxurl, data,function(response){	  
				...
				if(response.success){
					...
					var data = 'action=frs_render_row&post_id=' + response.id + '&nonce=' + nonce;
	...
	{
		var nonce = $('#frs-nonce').val();
		var ordr = $('table#table-slide tbody').sortable('serialize') + '&action=frs_list_update_order&nonce=' + nonce;
	...
	function frs_resort_data_table(){
	 	$('table#table-slide tbody').sortable({
		    ...
		    update: function() {
		    	var nonce = $('#frs-nonce').val();
		        var ordr = $(this).sortable('serialize') + '&action=frs_list_update_order&nonce=' + nonce;

			...
			switch( $type ) {
				...
				case 'icon':
					...
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picker-box' ?>" class="hoot-of-icon-picker-box">
						<div class="hoot-of-icon-picker-list"><i class="fa fa-ban hoot-of-icon-none" data-value="0" data-category=""><span><?php _e( 'Remove Icon', 'responsive-brix' ) ?></span></i></div>
						<?php
				...
				case 'image':
					...
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-image-selected' ?>" class="hoot-of-image-selected" data-title="<?php _e( 'Select Image', 'responsive-brix' ) ?>" data-update="<?php _e( 'Set Image', 'responsive-brix' ) ?>" data-library="image"><span class="hoot-of-image-selected-img" <?php
						if ( !empty( $value ) ) {
							$post = get_post( $value );
						...
						?>></span><span class="hoot-of-image-selected-label"><?php _e( 'Add Image', 'responsive-brix' ) ?></span></div>
						<a href="#" class="hoot-of-image-remove"><?php _e( 'Remove Image', 'responsive-brix' ) ?></a>
					<?php
					break;
				...
				case 'collapse':
					...
					?><div class="hoot-widget-field-collapse" data-id="<?php echo esc_attr( $id ) ?>">
						<div class="hoot-collapse-head"><h3><?php if ( !empty( $name ) ) echo esc_html($name); else _e('Group', 'responsive-brix') ?> <i class="fa fa-sort"></i></h3></div>
						<div class="hoot-collapse-body">
